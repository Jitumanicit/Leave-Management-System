					...
					<div class="portlet box yellow">
						<div class="portlet-title">
							<div class="caption">
								...
								<i class="fa fa-user"></i>Satff List 
                                </div>
                                <div align="right" style="padding-left:10px;">
						
						<?php 
						
						$xyz=datamanager::get_sum_tot('staff','salary','pric');
						foreach ($xyz as $keyxyz);
						echo 'Total='.$keyxyz['pric'].'&nbsp;&nbsp;&nbsp;';
						?>
						
						</div>
							<div align="right" style="padding:12px; font-weight:none; font-size:17px;"><a href="add_staff.php">Add New Staff</a></div>
						</div>
...
$delete_staff=$_GET['delete'];
{
							...
							for($i=0; $key3 = $sql4->fetch(); $i++)
						{
							...
							<!--<td><input type="checkbox" class="checkboxes" value="<?php echo $key3['id'];?>"/></td>-->
                                ...
                                <td><?php echo $key3['doj'];?></td>
                                <td><?php echo $key3['gender'];?></td>
	...
	$sql2->execute();
						...
						?>	
							...
							<tr class="odd gradeX">
								...
								<td><input type="date" name="doj" value="<?php echo $key3['doj'];?>"/></td>
                                <td><input class="new_input_text" type="text" name="gender" value="<?php echo $key3['gender'];?>"/></td>
...
if(!isset($_GET['delete']) && !isset($_GET['edit']))
{
					...
					$sql2 = $dbhi->prepare("SELECT * FROM  staff WHERE date BETWEEN '$from_date' AND '$to_date'  ORDER BY `id` DESC");
							}
							else
							{
